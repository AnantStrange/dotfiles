...
function TopBar:decide_titles()
	self.main_title = mp.get_property_native('filename/no-ext')
	self.alt_title = nil -- Disable alternative titles

	if not self.main_title or self.main_title == '' then
		self.main_title = t('No file') -- Fallback if no filename is available
	end
end
